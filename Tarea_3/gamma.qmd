...
alpha_tcl <- 2.5
lambda_tcl <- 1.8
muestras_n <- c(5, 10, 20, 30, 50, 100)

resultados_tcl <- data.frame()
for (n in muestras_n) {
  medias_muestrales <- replicate(1000, mean(rgamma(n, shape = alpha_tcl, rate = lambda_tcl)))
  
  media_teorica <- alpha_tcl / lambda_tcl
  varianza_teorica <- (alpha_tcl / (lambda_tcl^2)) / n
...
kable(resultados_tcl,
      caption = "Comparación de Medias y Varianzas Muestrales con Teóricas",  
      digits = 4,
      col.names = c("n", "Media Muestral", "Varianza Muestral", "Media Teórica", "Varianza Teórica")) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed")) 
...
for (n in muestras_n) {
  medias_muestrales <- replicate(1000, mean(rgamma(n, shape = alpha_tcl, rate = lambda_tcl)))
  media_teorica <- alpha_tcl / lambda_tcl
  sd_teorica <- sqrt((alpha_tcl / (lambda_tcl^2)) / n)

  hist(medias_muestrales, breaks = 30, probability = TRUE,
       main = paste("n =", n),
       xlab = "Media Muestral",
       col = "pink", border = "black")
  curve(dnorm(x, mean = media_teorica, sd = sd_teorica), 
...
# Simulación de 1000 sumas de n variables Exp(λ)
sumas <- replicate(1000, sum(rexp(n, rate = lambda)))

# Histograma de las sumas
hist(sumas, breaks = 30, probability = TRUE,
     main = paste("Suma de", n, "Exp(λ =", lambda, ")"),
     xlab = "Suma de Variables Aleatorias con Distribución Exponencial",
     col = "olivedrab2", border = "black")

# Curva teórica Gamma(n, λ)
curve(dgamma(x, shape = n, rate = lambda), 
      col = "red", lwd = 2, add = TRUE)

# Leyenda
legend("topright", legend = c("Densidad Teórica Gamma(5, 1.5)"),
       col = "red", lwd = 2)
...
for (n in n_10_20) {
  sumas_n  <- replicate(1000, sum(rexp(n, rate = lambda)))
  hist(sumas_n, breaks = 30, probability = TRUE,
       main = paste("Suma de", n, "Exp(λ =", lambda, ")"),
       xlab = "Suma de Variables Aleatorias con Distribución Exponencial",
       col = "olivedrab2", border = "black")
  curve(dgamma(x, shape = n, rate = lambda), 
        col = "red", lwd = 2, add = TRUE)
  legend("topright", legend = c(paste("Densidad Teórica Gamma(", n, ",", lambda, ")")),
         col = c("red"), lwd = 2)
}
```
