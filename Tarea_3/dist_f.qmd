...
gl <- 15 
n <- 10000
t_muestras <- rt(n, df = gl)
f_muestras <- t_muestras^2

hist(f_muestras, breaks = 35, probability = TRUE,
     main = paste("Histograma de muestras de F(1, n) | PDF teórica"),
     xlab = "x", col = "pink", xlim = c(0, max(f_muestras)))
curve(df(x, 1, gl), col = "red", lwd = 2, add = TRUE)
legend("topright", legend = c("Muestras simuladas", "PDF teórica"), col = c("magenta", "red"), lwd = 2)
...
b <- 15
n <- 10000
f_muestras <- rf(n, df1 = a, df2 = b)
inv_f_muestras <- 1 / f_muestras

hist(inv_f_muestras, breaks = 35, probability = TRUE,
     main = paste("Histograma de muestras de F(b, a) | PDF teórica"),
     xlab = "x", col = "lightgreen", xlim = c(0, max(inv_f_muestras)))
curve(df(x, b, a), col = "red", lwd = 2, add = TRUE)
legend("topright", legend = c("Muestras simuladas", "PDF teórica"), col = c("#762", "red"), lwd = 2)
```
...
b <- 20
n <- 10000
x_muestras <- rchisq(n, df = a)
y_muestras <- rchisq(n, df = b)
f_muestras <- (x_muestras / a) / (y_muestras / b)

hist(f_muestras, breaks = 35, probability = TRUE,
     main = paste("Histograma de muestras de F(a, b) | PDF teórica"),
     xlab = "x", col = "green", xlim = c(0, max(f_muestras)))
curve(df(x, a, b), col = "red", lwd = 2, add = TRUE)
legend("topright", legend = c("Muestras simuladas", "PDF teórica"), col = c("lightblue", "red"), lwd = 2)
```
