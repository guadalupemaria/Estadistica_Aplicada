...
set.seed(123)
gl <- c(5, 20, 50)
muestras <- seq(100, 10000, by = 100)
par(mfrow = c(3, 1))
for (n in gl) {
  medias_muestrales <- sapply(muestras, function(m) {
    muestra <- rchisq(m, df = n)
    mean(muestra)
  })
  plot(muestras, medias_muestrales, type = "l", col = "green",
       main = paste("Convergencia de la Media Muestral (n =", n, ")", "hacia la Media Teórica"),
...
par(mfrow = c(3, 1))
for (n in gl) {
  varianzas_muestrales <- sapply(muestras, function(m) {
    muestra <- rchisq(m, df = n)
    var(muestra)
  })
  plot(muestras, varianzas_muestrales, type = "l", col = "green",
       main = paste("Convergencia de la Varianza Muestral (n =", n, ")", "hacia la Varianza Teórica"),
       xlab = "Tamaño de la Muestra", ylab = "Varianza Muestral")
...
n_grande <- max(muestras)
set.seed(123)
for (n in gl) {
  muestra <- rchisq(n_grande, df = n)
  hist(muestra, breaks = 30, probability = TRUE, col = "lightblue",
       main = paste("Histograma de la Muestra (n =", n, ") con Densidad Teórica"),
       xlab = "x", ylab = "Densidad")
  curve(dchisq(x, df = n), col = "red", lwd = 2, add = TRUE)
  legend("topright", legend = c("Densidad Teórica"),
         col = c("red"), lty = c(1))
}
```
...
set.seed(123)
gl <- 10
tamaños_muestra <- c(5, 10, 30, 50, 100, 500)
par(mfrow = c(3, 2))
for (n in tamaños_muestra) {
  muestras <- replicate(1000, mean(rchisq(n, df = gl)))
  hist(muestras, breaks = 30, probability = TRUE, col = "#ACD342",
       main = paste("TCL para n =", n, "grados de libertad"),
       xlab = "Media Muestral", ylab = "Densidad")
  mu <- gl
  sigma <- sqrt(2 * gl / n)
  curve(dnorm(x, mean = mu, sd = sigma), col = "red", lwd = 2, add = TRUE)
  legend("topright", legend = c("Normal Teórica"),
         col = c("red"), lty = c(1))
}
```
...
# Simulación de la variable chi-cuadrada
df_exr <- data.frame(
  muestra = 1:muestras,
  Suma_var = replicate(muestras, sum(rnorm(n)^2))
)

# Visualización
ggplot(df_exr, aes(x = Suma_var)) +
  geom_histogram(aes(y = after_stat(density)), 
                 fill = "lightpink", color = "black", bins = 100, alpha = 0.7) +
  geom_function(fun = function(x) dchisq(x, df = n), 
                color = "firebrick", linewidth = 1.2) +
  labs(title = expression(paste("Distribución empírica vs. teórica de ", chi^2, "(5)")),
...
df_sumaji <- tibble(
  Muestra = 1:muestras,
  ji_1 = rchisq(muestras, n1),
  ji_2 = rchisq(muestras, n2),
  suma = ji_1 + ji_2
)

ggplot(df_sumaji)+
  geom_histogram(aes(suma, after_stat(density)), fill = "#374", alpha = 0.4, color = "brown", bins=80)+
  geom_function(fun = function(x) dchisq(x, n1+n2), linewidth = 1.2, color = "#679")+
  theme_bw()
...
n <- c(5, 10)
n_sim <- 1000
muestra_1 <- rchisq(n_sim, df = n[1])
muestra_2 <- rchisq(n_sim, df = n[2])
suma_muestras <- muestra_1 + muestra_2

df_suma <- data.frame(x = suma_muestras)
suma_ji <- ggplot(df_suma, aes(x = x)) +
  geom_histogram(aes(y = ..density..), bins = 30, fill = "magenta", color = "black", alpha = 0.7) +
  geom_function(fun = function(x) dchisq(x, df = sum(n)), aes(color = "Ji-cuadrada Teórica"), size = 1.4) +
  scale_color_manual(name = "Distribución",
                     values = c("Ji-cuadrada Teórica" = "green"),
                     breaks = c("Ji-cuadrada Teórica")) +
  labs(
    title = paste("Verificación de la propiedad de Aditividad de la distribución ji-cuadrada (n1 =", n[1], ", n2 =", n[2], ")"),
    subtitle = "Suma de dos variables aleatorias ji-cuadrada independientes",
    x = "Valor", y = "Densidad"
  ) +
  theme_minimal() + 
  theme(
