...
medias_muestrales <- numeric(length(tamaños_muestra))
...
df_convergencia_media <- data.frame(
  tamaño_muestra = tamaños_muestra,
  media_muestral = medias_muestrales
)

ggplot(df_convergencia_media, aes(x = tamaño_muestra, y = media_muestral)) +
  ...
  labs(
    ...
    subtitle = paste("Distribución Beta(", a, ",", b, ")"),
  ...
  annotate("text", x = max(tamaños_muestra) * 0.7, 
           y = (a / (a + b)) + 0.02, 
           label = paste("Media Teórica =", round(a / (a + b), 4)),
...
varianzas_muestrales <- numeric(length(tamaños_muestra))
for (i in seq_along(tamaños_muestra)) {
  n <- tamaños_muestra[i]
  muestra <- rbeta(n, a, b)
  varianzas_muestrales[i] <- var(muestra)
}
df_convergencia_varianza <- data.frame(
  tamaño_muestra = tamaños_muestra,
  varianza_muestral = varianzas_muestrales
)

ggplot(df_convergencia_varianza, aes(x = tamaño_muestra, y = varianza_muestral)) +
  ...
  annotate("text", x = max(tamaños_muestra) * 0.7, 
           y = ((a * b) / ((a + b)^2 * (a + b + 1))) + 0.002, 
           label = paste("Varianza Teórica =", 
                         round((a * b) / ((a + b)^2 * (a + b + 1)), 4)),
           color = "red", fontface = "bold")
```
...
n_grande <- max(tamaños_muestra)
muestra_grande <- rbeta(n_grande, a, b)
df_muestra_grande <- data.frame(x = muestra_grande)

ggplot(df_muestra_grande, aes(x = x)) +
  geom_histogram(aes(y = ..density..), bins = 30, fill = "darkmagenta", color = "black", alpha = 0.7) +
  ...
  annotate("text", x = 0.7, y = max(density(muestra_grande)$y) * 0.5, 
           label = "Curva Roja: PDF Teórica", color = "red", fontface = "bold")
```
...
a <- 2
b <- 5
tamaños_muestra_tcl <- c(5, 30, 100, 500)
n_replicas <- 1000

plots_tcl <- list()
for (n in tamaños_muestra_tcl) {
  medias_replicas <- numeric(n_replicas)
  for (i in 1:n_replicas) {
    muestra <- rbeta(n, a, b)
    medias_replicas[i] <- mean(muestra)
  }
  df_tcl <- data.frame(medias = medias_replicas)
  
  p_tcl <- ggplot(df_tcl, aes(x = medias)) +
    geom_histogram(aes(y = ..density..), bins = 30, 
                   fill = "magenta", color = "black", alpha = 0.7) +
    stat_function(fun = dnorm, 
                  args = list(mean = a / (a + b), 
                              sd = sqrt((a * b) / ((a + b)^2 * (a + b + 1)) / n)), 
    ...
    annotate("text", x = mean(medias_replicas), 
             y = max(density(medias_replicas)$y) * 0.8, 
             label = "Normal Teórica", color = "black", fontface = "bold")
  plots_tcl[[as.character(n)]] <- p_tcl
}
grid.arrange(grobs = plots_tcl, ncol = 2,
             top = "Verificación del Teorema Central del Límite con Beta(2,5)")
```
...
# Resumen de los resultados del TCL
cat("RESUMEN DE LA VERIFICACIÓN DEL TEOREMA CENTRAL DEL LÍMITE\n")
cat("=============================\n\n")
for (n in tamaños_muestra_tcl) {
  medias_replicas <- numeric(n_replicas)
  for (i in 1:n_replicas) {
  ...
  media <- mean(medias_replicas)
  varianza <- var(medias_replicas)
  sd <- sd(medias_replicas)
  media_teorica <- a / (a + b)
  varianza_teorica <- (a * b) / ((a + b)^2 * (a + b + 1)) / n
  sd_teorica <- sqrt(varianza_teorica)
  
  cat(sprintf("Tamaño de Muestra: %d\n", n))
  cat(sprintf("Media: %.4f | Media Teórica: %.4f\n", 
              media, media_teorica))
  cat(sprintf("Varianza: %.4f | Varianza Teórica: %.4f\n",
              varianza, varianza_teorica))
  cat(sprintf("Desviación Estándar: %.4f | Desviación Estándar Teórica: %.4f\n\n", 
              sd, sd_teorica))
}
```
