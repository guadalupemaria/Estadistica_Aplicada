...
```{r}
#| code-fold: true
#| label: simulacion_geom_1

# Parámetro de éxito (cara)
p <- 0.5

# Número de simulaciones
n_sim <- 10000

# Simular lanzamientos hasta la primera cara
set.seed(123)
muestra_geom <- rgeom(n_sim, prob = p) + 1  

# a) Probabilidad de necesitar exactamente 5 lanzamientos
prob_5 <- mean(muestra_geom == 5)
cat("a) P(X = 5):", round(prob_5, 4), "\n")

# b) Probabilidad de necesitar más de 3 lanzamientos
prob_mas_3 <- mean(muestra_geom > 3)
cat("b) P(X > 3):", round(prob_mas_3, 4), "\n")

# c) Media y varianza empírica
media_emp <- mean(muestra_geom)
varianza_emp <- var(muestra_geom)
cat("c) Media empírica:", round(media_emp, 4), "\n")
cat("   Varianza empírica:", round(varianza_emp, 4), "\n")
...
# Frecuencia relativa
df <- as.data.frame(table(muestra_geom))
colnames(df) <- c("Lanzamientos", "Frecuencia")
df$Probabilidad <- df$Frecuencia / sum(df$Frecuencia)

# Graficar función de masa de probabilidad
ggplot(df, aes(x = as.numeric(Lanzamientos), y = Probabilidad)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  labs(title = "Distribución Geométrica Simulada (p = 0.5)",
       subtitle = "Número de lanzamientos hasta la primera cara",
...
# Parámetro de éxito (anotar)
p <- 0.7

# Número de simulaciones
n_sim <- 10000

# Simular intentos hasta el primer acierto
set.seed(123)
muestra_geom <- rgeom(n_sim, prob = p) + 1  

# a) Probabilidad de anotar en el tercer intento
prob_3 <- mean(muestra_geom == 3)
cat("a) P(X = 3):", round(prob_3, 4), "\n")

# b) Media empírica
media_emp <- mean(muestra_geom)
cat("b) Media empírica:", round(media_emp, 4), "\n")

# c) Probabilidad de necesitar entre 2 y 5 intentos
prob_2_5 <- mean(muestra_geom >= 2 & muestra_geom <= 5)
cat("c) P(2 ≤ X ≤ 5):", round(prob_2_5, 4), "\n")

```

##Visualización de la distribución geométrica

```{r}

# Frecuencia relativa
df <- as.data.frame(table(muestra_geom))
colnames(df) <- c("Intentos", "Frecuencia")
df$Probabilidad <- df$Frecuencia / sum(df$Frecuencia)

# Graficar función de masa de probabilidad
ggplot(df, aes(x = as.numeric(Intentos), y = Probabilidad)) +
  geom_bar(stat = "identity", fill = "darkgreen") +
  labs(title = "Distribución Geométrica Simulada (p = 0.7)",
       subtitle = "Intentos hasta el primer acierto",
       x = "Número de intentos",
       y = "Probabilidad") +
  theme_minimal()

```
La variable geométrica modela el número de intentos hasta el primer éxito.

La simulación confirma que los resultados empíricos convergen a los teóricos.

La forma de la distribución es decreciente, con mayor probabilidad en los primeros intentos.
...
a) Sea \( X \sim \text{Geom}(0.02) \), donde \( X \) representa el número de inspecciones necesarias hasta encontrar la primera pieza defectuosa. La función de masa de probabilidad está dada por:



\[
$$f(x) = P(X = x) = (1 - p)^{x - 1} \cdot p, \quad \text{para } x = 1, 2, 3, \ldots$$
\]
...
Por lo tanto, se espera inspeccionar en promedio 50 piezas antes de encontrar la primera defect

```{r}
# b) 
p <- 0.02
prob_mas_100 <- (1 - p)^100
round(prob_mas_100, 4)  # Resultado ≈ 0.1326

```

c) Sea \( X \sim \text{Geom}(0.02) \), donde \( X \) representa el número de inspecciones necesarias hasta encontrar la primera pieza defectuosa. La función de masa de probabilidad está dada por:
...
\textbf{a)} La esperanza matemática de \( X \) es:
...
Por lo tanto, se espera inspeccionar en promedio 50 piezas antes de encontrar la primera defectuosa.

\vspace{1em}

\textbf{b)} La probabilidad de que la primera pieza defectuosa aparezca después de la inspección número 100 es:



\[
$$P(X > 100) = (1 - p)^{100} = 0.98^{100} \approx 0.1326$$
\]
...
\textbf{c)} La distribución geométrica cumple la propiedad de \textit{falta de memoria}, es decir:
...
Para este problema, si ya se inspeccionaron 100 piezas sin encontrar ninguna defectuosa, la probabilidad de que aún se necesiten más de 20 inspecciones adicionales es:


\[
$$P(X > 120 \mid X > 100) = \frac{P(X > 120)}{P(X > 100)} = \frac{0.98^{120}}{0.98^{100}} = 0.98^{20}$$ 
$$= P(X > 20)$$
\]
...
s <- 50
t <- 30
prob_s_t <- 1 - pgeom(s + t-1, p_defecto)
prob_s <- 1 - pgeom(s-1, p_defecto)
prob_t <- 1 - pgeom(t-1, p_defecto)
prob_condicional <- prob_s_t / prob_s
cat("Demostración de la Propiedad de Falta de Memoria:\n")
cat("P(X  ≥", s+t, ") =", round(prob_s_t, 4), "\n")
cat("P(X  ≥", s, ") =", round(prob_s, 4), "\n")
cat("P(X  ≥", t, ") =", round(prob_t, 4), "\n\n")
cat("P(X  ≥", s+t, "| X  ≥", s, ") =", round(prob_condicional, 4), "\n")
cat("P(X  ≥", t, ") =", round(prob_t, 4), "\n\n")
cat("¿Son iguales?", abs(prob_condicional - prob_t) < 1e-10, "\n")
```

Esto confirma que la distribución geométrica no depende del historial de inspecciones previas.
