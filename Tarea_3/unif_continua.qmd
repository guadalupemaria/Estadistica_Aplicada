...
## Simulación de tiempos de carga
...
calificaciones <- runif(n_estud, min = a_calificacion, max = b_calificacion)
...
porcentajes_teoricos <- c(
  Aprobado = (79 - 70) / (b_calificacion - a_calificacion) * 100,
  Bien = (89 - 80) / (b_calificacion - a_calificacion) * 100,
  Excelente = (100 - 90) / (b_calificacion - a_calificacion) * 100
)
...
ggplot(df_calificaciones, aes(x = calificaciones, fill = categoria)) +
  ...
  scale_fill_manual(values = c("Aprobado" = "violet", "Bien" = "lightblue", "Excelente" = "lightyellow")) +
...
ggplot(df_tcl_truncado, aes(x = medias)) +
  geom_histogram(aes(y = after_stat(density)), bins = 30,
                 fill = "lightgreen", color = "black", alpha = 0.7) +
  geom_vline(xintercept = mean(df_tcl_truncado$medias), color = "violet", size = 1, linetype = "dashed") +
  facet_wrap(~n_factor, scales = "free_y", ncol = 3) +
...
ggplot(df_pi, aes(x = x, y = y, color = dentro)) +
  ...
  coord_fixed() +
  labs(title = "Simulación de Monte Carlo para Estimar pi",
       subtitle = paste("Estimación de pi =", round(pi_estimado, 5)),
       x = "X",
       y = "Y",
       color = "Centro del Círculo") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5),
        plot.subtitle = element_text(hjust = 0.5)) +
  scale_color_manual(values = c("TRUE" = "lightblue", "FALSE" = "lightpink"))
