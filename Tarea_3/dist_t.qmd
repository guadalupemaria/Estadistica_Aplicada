...
ggplot(df_muestra, aes(x = x)) +
  geom_histogram(aes(y = ..density..), bins = 30, fill = "#176349", color = "black", alpha = 0.7) +
  stat_function(fun = dt, args = list(df = gl), color = "red", size = 1.2, 
                aes(linetype = "t(5)")) +
  stat_function(fun = dnorm, args = list(mean = 0, sd = 1), color = "#111273", size = 1.2, 
                aes(linetype = "N(0,1)")) +
  labs(
